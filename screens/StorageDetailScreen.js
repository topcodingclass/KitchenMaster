...
const StorageDetailScreen = ({ route, navigation }) => {
  const { storage, foods: passedFoods } = route.params; // for virtual storage
  ...
  const fetchItems = async () => {
    try {
      if (storage.isVirtual && passedFoods) {
        // Use passed foods for virtual storage
        setItems(passedFoods);
      } else {
  ...
  const deleteStorage = async () => {
    if (storage.isVirtual) {
      alert("Cannot delete this storage.");
      return;
    }
    if (items.length > 0) {
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <View style={{ paddingBottom: 20 }}>
        <Button icon="delete" mode="contained" onPress={deleteStorage} disabled={storage.isVirtual}>
          Delete Storage
        </Button>
      </View>
